...
namespace Octopus.Server.Extensibility.JiraIntegration.Configuration
{
    internal class JiraConfigurationSettings :
        ExtensionConfigurationSettings<JiraConfiguration, JiraConfigurationResource, IJiraConfigurationStore>,
        IJiraConfigurationSettings
    {
        ...
        public override IEnumerable<IConfigurationValue> GetConfigurationValues()
        {
            ...
            yield return new ConfigurationValue<bool>("Octopus.JiraIntegration.IsEnabled", isEnabled, isEnabled,
                "Is Enabled");
            yield return new ConfigurationValue<string?>("Octopus.JiraIntegration.BaseUrl",
                ConfigurationDocumentStore.GetBaseUrl(),
                isEnabled && !string.IsNullOrWhiteSpace(ConfigurationDocumentStore.GetBaseUrl()), "Jira Base Url");
            yield return new ConfigurationValue<SensitiveString?>("Octopus.JiraIntegration.ConnectAppPassword",
                ConfigurationDocumentStore.GetConnectAppPassword(),
                isEnabled && !string.IsNullOrWhiteSpace(ConfigurationDocumentStore.GetConnectAppPassword()?.Value),
                "Jira Connect App Password");
            yield return new ConfigurationValue<string?>("Octopus.JiraIntegration.Username",
                ConfigurationDocumentStore.GetJiraUsername(),
                isEnabled && !string.IsNullOrWhiteSpace(ConfigurationDocumentStore.GetJiraUsername()), "Jira Username");
            yield return new ConfigurationValue<SensitiveString?>("Octopus.JiraIntegration.Password",
                ConfigurationDocumentStore.GetJiraPassword(),
                isEnabled && !string.IsNullOrWhiteSpace(ConfigurationDocumentStore.GetJiraPassword()?.Value),
                "Jira Password");
            yield return new ConfigurationValue<string?>("Octopus.JiraIntegration.IssueTracker.JiraReleaseNotePrefix",
                ...
                isEnabled && !string.IsNullOrWhiteSpace(ConfigurationDocumentStore.GetReleaseNotePrefix()),
