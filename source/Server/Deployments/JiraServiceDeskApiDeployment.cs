...
{
    internal class JiraServiceDeskApiDeployment : IJiraApiDeployment
    {
        private readonly string jiraServiceId;
        ...
        public string[] DeploymentValues(DeploymentResource deployment)
        {
            if (string.IsNullOrEmpty(jiraServiceId))
                throw new JiraDeploymentException(
                    "Service ID is empty. Please supply a Jira Service Desk Service ID and try again");
