...
namespace Octopus.Server.Extensibility.JiraIntegration.Integration
{
    internal class JiraRestClient : IJiraRestClient, IDisposable
    {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        if (permissionsContainer == null)
                        {
                            connectivityCheckResponse.AddMessage(ConnectivityCheckMessageCategory.Error,
                                "Unable to read permissions from response body");
        ...
        public async Task<IResultFromExtension<JiraIssue[]>> GetIssues(string[] workItemIds)
        {
            ...
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (result == null)
                    {
                        ...
                        return ResultFromExtension<JiraIssue[]>.Failed(
                            "Jira Work Item data not found in response body");
                    }
                ...
                errorMessage =
                    $"Failed to retrieve Jira issues from {baseUrl}. Response Code: {response.StatusCode}{(errorResult?.ErrorMessages.Any() == true ? $" (Errors: {string.Join(", ", errorResult.ErrorMessages)})" : "")}";
            }
            catch (HttpRequestException e)
            {
                errorMessage =
    ...
    internal class JiraErrorResult
    {
        [JsonProperty("errorMessages")] public string[] ErrorMessages { get; set; } = Array.Empty<string>();
    }
